...
if(isset($_POST['get_all_rooms']))
{
  ...
  {
    if($row['status'] == 1){
       $status = "<button onclick='toggle_status($row[id],0)' class='btn btn-sm btn-success shadow-none'>active</button>
       ";
    }
    else{
      $status = "<button onclick='toggle_status($row[id],1)' class='btn btn-sm btn-warning text-white shadow-none'>inactive</button>
      ";
    }
...
{
    $frm_data = filteration($_POST);
    $values = [$frm_data['rem_image'],$frm_data['room_id']];
